...
//PLNSigner is used by the Mapping Service to sign messages as well as verify signatures
...
func (m *PLNSigner) Init(ctx context.Context, Msgr infra.Messenger,
	...
	t, err := m.getTRC()
	if err != nil {
		return serrors.WrapStr("Error in init PLNSigner", err)
	}
	m.signedTRC = t
