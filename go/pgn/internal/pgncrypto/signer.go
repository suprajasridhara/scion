...
//PGNSigner is used by the Mapping Service to sign messages as well as verify signatures
type PGNSigner struct {
...
//Init initializes PGNSigner
func (m *PGNSigner) Init(ctx context.Context, Msgr infra.Messenger,
	IA addr.IA, cfgDir string) error {
	m.Msgr = Msgr
	m.IA = IA
	t, err := m.getTRC()
	if err != nil {
		return serrors.WrapStr("Error in init PGNSigner", err)
	}
	m.signedTRC = t
...
func (m *PGNSigner) getChains(ctx context.Context,
...
func (m *PGNSigner) getTRC() (cppki.SignedTRC, error) {
	addr := &snet.SVCAddr{IA: m.IA, SVC: addr.SvcCS}
